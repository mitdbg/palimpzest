...
do
  ...
  do
    ...
    if [ -f $FILE ]; then
      ...
      python demos/cuad-demo.py --constrained --k $k --j $j --sample-budget $sample_budget --seed $seed --exp-name $exp_name
    fi
    ...
    if [ -f $FILE ]; then
      ...
      python demos/cuad-demo.py --constrained --priors-file cuad-priors.json --k $k --j $j --sample-budget $sample_budget --seed $seed --exp-name $exp_name
    fi
    ...
    if [ -f $FILE ]; then
      ...
      python demos/cuad-demo.py --constrained --priors-file cheap-priors.json --k $k --j $j --sample-budget $sample_budget --seed $seed --exp-name $exp_name
    fi
  done
done
