...
for cost in 0.5 # 0.75 1.0 1.25 1.50
do
  for seed in {0..9}
  do
    ...
    do
      ...
      do
        ...
        if [ -f $FILE ]; then
          ...
          python demos/biodex-pareto-cascades.py --progress --k $k --j $j --sample-budget $budget --optimizer-strategy $strategy --cost $cost --seed $seed --exp-name $exp_name
        fi
        ...
        if [ -f $FILE ]; then
          ...
          python demos/biodex-pareto-cascades.py --progress --priors-file biodex-priors-cascades.json --k $k --j $j --sample-budget $budget --optimizer-strategy $strategy --cost $cost --seed $seed --exp-name $exp_name
        fi
        ...
        if [ -f $FILE ]; then
          ...
          python demos/biodex-pareto-cascades.py --progress --priors-file cheap-priors-cascades.json --k $k --j $j --sample-budget $budget --optimizer-strategy $strategy --cost $cost --seed $seed --exp-name $exp_name
        fi
      done
    done
  done
