...
class SequentialSingleThreadSentinelPlanExecutor(ExecutionEngine):
    ...
    def execute_plan(self, plan: SentinelPlan,
        ...
        for op_set in plan.operator_sets:
            ...
            op_set_details = {
                op.op_name(): {k: v for k, v in op.get_op_params().items() if not isinstance(v, Schema)}
                for op in op_set
            }
    ...
    def execute_plan(self, plan: SentinelPlan,
        ...
        for op_set in plan.operator_sets:
            for op in op_set:
                op_id = op.get_op_id()
                op_name = op.op_name()
                op_details = {k: v for k, v in op.get_op_params().items() if not isinstance(v, Schema)} # TODO
