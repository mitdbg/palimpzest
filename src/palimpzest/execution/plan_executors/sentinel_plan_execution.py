...
class SequentialParallelSentinelPlanExecutor(ExecutionEngine):
    """
    """
    def execute_plan(self, plan: SentinelPlan, num_samples: int, plan_workers: int = 1):
        """Initialize the stats and the execute the plan."""
        if self.verbose:
            print("----------------------")
            print(f"PLAN[{plan.plan_id}] (n={num_samples}):")
        ...
        for op_set in plan.operator_sets:
            # create a sample matrix
            sample_matrix = create_sample_matrix(len(candidates), len(op_set), self.rank)

            # for non-zero entries in sample matrix: create future task

            # run futures over worker pool
