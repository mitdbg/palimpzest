...
class GenerationStats(BaseModel):
    ...
    # the number of text tokens read from the cache (charged at a discount, typically 0.1x input rate)
    text_cache_read_tokens: int = 0

    # the number of audio tokens read from the cache. Palimpzest does not currently support this; added here for future implementation.
    audio_cache_read_tokens: int = 0

    # the number of image tokens read from cache. LLMs do not currently support this; added here for future implementation.
    image_cache_read_tokens: int = 0

    # the total number of tokens read from cache.
    total_cache_read_tokens: int = 0

    # the number of text tokens written to the cache (charged at creation rate, typically 1.25x input rate)
    text_cache_creation_tokens: int = 0

    # the number of audio tokens written to the cache (Anthropic only). Palimpzest does not currently support this; added here for future implementation.
    audio_cache_creation_tokens: int = 0

    # the number of image tokens written to the cache (Anthropic only). LLMs do not currently support this; added here for future implementation.
    image_cache_creation_toknes: int = 0
    ...
    # the total cost associated with reading tokens from the cache (text + audio)
    total_cache_read_cost: float = 0.0

    # the total cost associated with writing tokens to the cache (text + audio)
