...
class GenerationStats(BaseModel):
    ...
    input_text_tokens: float = 0

    # the total number of input audio tokens processed by this operation.
    input_audio_tokens: float = 0

    # the total number of input image tokens processed by this operation.
    input_image_tokens: float = 0

    # the total number of text cache read tokens processed by this operation (charged at a discount, typically 0.1x input rate)
    text_cache_read_tokens: float = 0

    # the total number of audio cache read tokens processed by this operation. Palimpzest does not currently support this; added here for future implementation.
    audio_cache_read_tokens: float = 0

    # the total number of image cache read tokens processed by this operation. LLMs do not currently support this; added here for future implementation.
    image_cache_read_tokens: float = 0

    # the total number of text tokens written to the cache in this operation (Anthropic only) (charged at creation rate, typically 1.25x input rate)
    text_cache_creation_tokens: int = 0

    # the total number of audio tokens written to the cache in this operation (Anthropic only). Palimpzest does not currently support this; added here for future implementation.
    audio_cache_creation_tokens: float = 0

    # the total number of image tokens written to the cache in this operation (Anthropic only). LLMs do not currently support this; added here for future implementation.
    image_cache_creation_tokens: float = 0

    # the number of output audio tokens generated by the model
...
class RecordOpStats(BaseModel):
    ...
    # the number of text cache read tokens processed by this operation
    text_cache_read_tokens: float = 0.0

    # the number of audio cache read tokens processed by this operation
    audio_cache_read_tokens: float = 0.0

    # the number of image cache read tokens processed by this operation
    image_cache_read_tokens: float = 0.0

    # the number of text tokens written to cache by this operation
    text_cache_creation_tokens: float = 0.0

    # the number of audio tokens written to cache by this operation
    audio_cache_creation_tokens: float = 0.0

    # the number of image tokens written to cache by this operation
    image_cache_creation_tokens: float = 0.0

    # the number of output audio tokens generated by this operation
...
class OperatorStats(BaseModel):
    ...
    def __iadd__(self, stats: OperatorStats | RecordOpStats) -> OperatorStats:
        ...
        if isinstance(stats, OperatorStats):
            ...
            self.text_cache_read_tokens += stats.text_cache_read_tokens
            self.audio_cache_read_tokens += stats.audio_cache_read_tokens
            self.image_cache_read_tokens += stats.image_cache_read_tokens
            self.text_cache_creation_tokens += stats.text_cache_creation_tokens
            self.audio_cache_creation_tokens += stats.audio_cache_creation_tokens
            self.image_cache_creation_tokens += stats.image_cache_creation_tokens
            self.output_audio_tokens += stats.output_audio_tokens
...
class BasePlanStats(BaseModel):
    ...
    # text cache read tokens processed by this plan
    text_cache_read_tokens: float = 0.0

    # audio cache read tokens processed by this plan
    audio_cache_read_tokens: float = 0.0

    # image cache read tokens processed by this plan
    image_cache_read_tokens: float = 0.0

    # text tokens written to cache by this plan
    text_cache_creation_tokens: float = 0.0

    # audio tokens written to cache by this plan
    audio_cache_creation_tokens: float = 0.0

    # image tokens written to cache by this plan
    image_cache_creation_tokens: float = 0.0

    # output audio tokens generated by this plan
    output_audio_tokens: float = 0.0
    ...
    def finish(self) -> None:
        ...
        self.text_cache_read_tokens = self.sum_op_stats_field("text_cache_read_tokens") + self.sum_validation_stats_field("text_cache_read_tokens")
        self.audio_cache_read_tokens = self.sum_op_stats_field("audio_cache_read_tokens") + self.sum_validation_stats_field("audio_cache_read_tokens")
        self.image_cache_read_tokens = self.sum_op_stats_field("image_cache_read_tokens") + self.sum_validation_stats_field("image_cache_read_tokens")
        self.text_cache_creation_tokens = self.sum_op_stats_field("text_cache_creation_tokens") + self.sum_validation_stats_field("text_cache_creation_tokens")
        self.audio_cache_creation_tokens = self.sum_op_stats_field("audio_cache_creation_tokens") + self.sum_validation_stats_field("audio_cache_creation_tokens")
        self.image_cache_creation_tokens = self.sum_op_stats_field("image_cache_creation_tokens") + self.sum_validation_stats_field("image_cache_creation_tokens")
        self.output_audio_tokens = self.sum_op_stats_field("output_audio_tokens") + self.sum_validation_stats_field("output_audio_tokens")
...
class PlanStats(BasePlanStats):
    ...
    def __iadd__(self, plan_stats: PlanStats) -> None:
        ...
        self.text_cache_read_tokens += plan_stats.text_cache_read_tokens
        self.audio_cache_read_tokens += plan_stats.audio_cache_read_tokens
        self.image_cache_read_tokens += plan_stats.image_cache_read_tokens
        self.text_cache_creation_tokens += plan_stats.text_cache_creation_tokens
        self.audio_cache_creation_tokens += plan_stats.audio_cache_creation_tokens
        self.image_cache_creation_tokens += plan_stats.image_cache_creation_tokens
        self.output_audio_tokens += plan_stats.output_audio_tokens
    ...
    def __str__(self) -> str:
        ...
        stats += f"text_cache_read_tokens={self.text_cache_read_tokens} \n"
        stats += f"audio_cache_read_tokens={self.audio_cache_read_tokens} \n"
        stats += f"image_cache_read_tokens={self.image_cache_read_tokens} \n"
        stats += f"text_cache_creation_tokens={self.text_cache_creation_tokens} \n"
        stats += f"audio_cache_creation_tokens={self.audio_cache_creation_tokens} \n"
        stats += f"image_cache_creation_tokens={self.image_cache_creation_tokens} \n"
        stats += f"output_audio_tokens={self.output_audio_tokens} \n"
...
class ExecutionStats(BaseModel):
    ...
    # text cache read tokens processed
    text_cache_read_tokens: float = 0.0

    # audio cache read tokens processed
    audio_cache_read_tokens: float = 0.0

    # image cache read tokens processed
    image_cache_read_tokens: float = 0.0

    # text tokens written to cache
    text_cache_creation_tokens: float = 0.0

    # audio tokens written to cache
    audio_cache_creation_tokens: float = 0.0

    # image tokens written to cache
    image_cache_creation_tokens: float = 0.0

    # output audio tokens generated
    output_audio_tokens: float = 0.0
    ...
    def finish(self) -> None:
        ...
        self.text_cache_read_tokens = self.sum_plan_stats_field("text_cache_read_tokens")
        self.audio_cache_read_tokens = self.sum_plan_stats_field("audio_cache_read_tokens")
        self.image_cache_read_tokens = self.sum_plan_stats_field("image_cache_read_tokens")
        self.text_cache_creation_tokens = self.sum_plan_stats_field("text_cache_creation_tokens")
        self.audio_cache_creation_tokens = self.sum_plan_stats_field("audio_cache_creation_tokens")
        self.image_cache_creation_tokens = self.sum_plan_stats_field("image_cache_creation_tokens")
        self.output_audio_tokens = self.sum_plan_stats_field("output_audio_tokens")
