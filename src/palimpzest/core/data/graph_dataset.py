...
class GraphDataset(Dataset):
    ...
    def __init__(self, *, graph_id: str | None = None, name: str | None = None, store: GraphStore | None = None) -> None:
        self.name = name
        self.graph_id = graph_id or str(uuid.uuid4())
        self.revision: int = 0
        self.store = store or MemoryGraphStore()
        self._node_keys: list[str] | None = None

        self._induction_log: InductionLog = InductionLog()
        
        # Initialize as a Dataset source
        # We use GraphNode as the schema for the dataset (it yields nodes)
        super().__init__(sources=None, operator=BaseScan(datasource=self, output_schema=GraphNode), schema=GraphNode, id=self.graph_id)
    ...
    def __getitem__(self, idx: int) -> dict:
        if self._node_keys is None:
            self._node_keys = sorted(self.store.get_node_ids())

        node_id = self._node_keys[idx]
        node = self.store.get_node(node_id)
        return node.model_dump()

    def add_node(self, node: GraphNode, *, overwrite: bool = False) -> None:
        self.store.add_node(node, overwrite=overwrite)
        self._bump_revision()
    ...
    def add_edge(self, edge: GraphEdge, *, overwrite: bool = False) -> None:
        self.store.add_edge(edge, overwrite=overwrite)
        self._bump_revision()
    ...
    def _remove_edge(self, edge_id: str, *, bump_revision: bool) -> None:
        # Deprecated internal helper, mapped to public remove_edge
        self.store.remove_edge(edge_id)
        if bump_revision:
            self._bump_revision()
    ...
    def iter_out_edges(self, node_id: str, *, edge_type: str | None = None) -> Iterable[GraphEdge]:
        return self.store.iter_out_edges(node_id, edge_type=edge_type)

    def iter_in_edges(self, node_id: str, *, edge_type: str | None = None) -> Iterable[GraphEdge]:
        return self.store.iter_in_edges(node_id, edge_type=edge_type)
    ...
    def iter_neighbors(self, node_id: str, *, edge_type: str | None = None) -> Iterable[tuple[GraphEdge, GraphNode]]:
        ...
        for edge in self.iter_out_edges(node_id, edge_type=edge_type):
            ...
            yield edge, self.get_node(neighbor_id)
    ...
    def to_snapshot(self, *, include_overlay: bool = True) -> GraphSnapshot:
        ...
        return GraphSnapshot(
            ...
            nodes=list(self.store.iter_nodes()),
            edges=edges,
    ...
    def from_snapshot(cls, snapshot: GraphSnapshot, *, name: str | None = None) -> GraphDataset:
        ...
        # Bulk load into store
        graph.store.add_nodes(snapshot.nodes)
        graph.store.add_edges(snapshot.edges)
    ...
    def _impacted_node_ids(self, *, spec_id: str, mode: str) -> set[str]:
        node_ids = set(self.store.get_node_ids())
        if mode == "full":
            return node_ids
        if mode != "incremental":
        ...
        if entry.last_revision is not None:
            ...
            for node in self.store.iter_nodes():
                if (getattr(node, "revision", 0) or 0) > entry.last_revision:
    ...
    def run_knn_similarity_induction(
        ...
        *,
        ...
        all_node_ids = list(self.store.get_node_ids())
        valid_node_ids = [nid for nid in all_node_ids if has_embedding(nid)]
        valid_impacted = {nid for nid in impacted if nid in set(valid_node_ids)}

        if not valid_impacted:
            # Nothing to do; still advance the log to avoid re-processing nodes with missing embeddings.
            self._induction_log.upsert(
                InductionLogEntry(
                    spec=spec,
                    processed_node_ids=sorted(all_node_ids),
        ...
        entry = InductionLogEntry(
            spec=spec,
            processed_node_ids=sorted(self.store.get_node_ids()),
            last_revision=self.revision,
        )
    ...
    def _candidate_pairs_dataset(self, *, spec_id: str, generator_kind: str, generator_params: dict[str, Any], impacted: set[str]):
        ...
        if generator_kind == "all_pairs":
            ...
            node_ids = list(self.store.get_node_ids())
    ...
    def run_predicate_induction(self, *, spec_id: str, mode: str = "full"):
        ...
        if mode == "incremental" and entry.spec.incremental_mode == "bidirectional" and not entry.spec.symmetric:
            ...
            old_node_ids = set(self.store.get_node_ids()) - impacted
            if old_node_ids:
