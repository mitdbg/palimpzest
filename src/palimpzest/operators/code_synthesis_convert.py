...
from palimpzest.operators.convert import (
    LLMConvert,
    LLMConvertBonded,
...
class CodeSynthesisConvert(LLMConvert):
    def __init__(
        ...
        **kwargs,
        ...
        self.code_champion_generator = generators.CustomGenerator(
            model_name=self.code_synth_model.value
        )
    ...
    def __eq__(self, other: CodeSynthesisConvert):
        return (
            ...
            and self.exemplar_generation_model
            == other.exemplar_generation_model
            and self.code_synth_model == other.code_synth_model
            and self.conventional_fallback_model
            == other.conventional_fallback_model
    ...
    def naiveCostEstimates(
        ...
        naive_op_cost_estimates.cost_per_record = (
            1e-4  # amortize code synth cost across records
        )
        naive_op_cost_estimates.quality = (naive_op_cost_estimates.quality) * (
            GPT_4_MODEL_CARD["code"] / 100.0
        )

        return naive_op_cost_estimates

    def _fetch_cached_code(
        self, fields_to_generate: List[str]
    ) -> Tuple[Dict[CodeName, Code]]:
        ...
        for field_name in fields_to_generate:
            ...
            code_ensemble = cache.getCachedData(
                "codeEnsembles", code_ensemble_cache_id
            )
        ...
        if all(
            [
                field_to_code_ensemble.get(field_name, None) is not None
                for field_name in fields_to_generate
            ]
        ):
    ...
    def synthesize_code_ensemble(
        ...
        **kwargs,
        ...
        for field_name in fields_to_generate:
            ...
            code_ensemble, code_synth_stats = self._synthesize_field_code(
                api, field_name
            )
            ...
            if self.cache_across_plans:
                cache = DataDirectory().getCacheService()
                code_ensemble_cache_id = "_".join(
                    [self.get_op_id(), field_name]
                )
                cache.putCachedData(
                    "codeEnsembles", code_ensemble_cache_id, code_ensemble
                )
    ...
    def _bonded_query_fallback(self, candidate, start_time):
        ...
        field_answers, generation_stats = bonded_op.convert(
            candidate_content, fields_to_generate
        )
        ...
        exemplars = [
            (candidate_dict, dr._asDict(include_bytes=False)) for dr in drs
        ]
    ...
    def __call__(self, candidate):
        ...
        if self._shouldSynthesize():
            ...
            if self.cache_across_plans:
                self.field_to_code_ensemble = self._fetch_cached_code(
                    fields_to_generate
                )
        ...
        for field_name in fields_to_generate:
            ...
            answer, exec_stats = generators.codeEnsembleExecution(
            ...
            if answer is not None:
                ...
                json_answers, field_stats = conventional_op.convert(
                    candidate_content, [field_name]
                )

                # include code execution time in field_stats
                field_stats.fn_call_duration_secs += (
                    exec_stats.fn_call_duration_secs
                )
...
class CodeSynthesisConvertSingle(CodeSynthesisConvert):
    def _shouldSynthesize(
        self, num_exemplars: int = 1, *args, **kwargs
    ) -> bool:
    ...
    def _code_synth_single(
        ...
        context = {
            ...
            "inputs_desc": "\n".join(
                [
                    f"- {field_name} ({api.input_descs[i]})"
                    for i, field_name in enumerate(api.inputs)
                ]
            ),
            ...
            "examples_desc": "\n".join(
                [
                    EXAMPLE_PROMPT.format(
                        ...
                        example_inputs="\n".join(
                            [
                                f"- {field_name} = {repr(example[0][field_name])}"
                                for field_name in api.inputs
                            ]
                        ),
        ...
        **kwargs,
        ...
        for i in range(code_ensemble_num):
            ...
            code, stats = self._code_synth_single(
                api, output_field_name, exemplars=[exemplar]
            )
...
class CodeSynthesisConvertAdviceEnsemble(CodeSynthesisConvert):
    ...
    def _synthesize_advice(
        ...
        advs = self._parse_multiple_outputs(
            pred, outputs=[f"Idea {i}" for i in range(1, limit + 1)]
        )
    ...
    def _synthesize_field_code(
        ...
        **kwargs,
        ...
        advices, adv_stats = self._synthesize_advice(
            api,
            output_field_name,
            exemplars=self.exemplars[:num_exemplars],
        ...
        for key, value in adv_stats.items():
            if type(value) == type(dict()):
                ...
                output_stats[key] += (
                    output_stats.get(key, type(value)()) + value
                )
...
class CodeSynthesisConvertAdviceEnsembleValidation(CodeSynthesisConvert):
    def _shouldSynthesize(
        self, code_regenerate_frequency: int = 200, *args, **kwargs
    ):
