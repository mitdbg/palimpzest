...
class PhysicalOp:
    ...
    def __init__(
        self, outputSchema: Schema, source: PhysicalOp = None, shouldProfile=False
    ) -> None:
...
class Induce(PhysicalOp):
    ...
    def __init__(
        ...
        shouldProfile=False,
        max_workers=1,  # TODO should max_workers be a general property of PhysicalOp, or a special property of even subclasses?
    ):
        super().__init__(
...
class InduceFromCandidateOp(Induce):
...
class ParallelInduceFromCandidateOp(Induce):
...
class FilterCandidateOp(PhysicalOp):
    ...
    def __str__(self):
        ...
        return (
            ...
            + model_str
            + ", Prompt Strategy: "
            + str(self.prompt_strategy.value)
            + ")"
        )
    ...
    def estimateCost(self, cost_est_data: Dict[str, Any] = None):
        ...
        # make sure to use input cardinality (not output cardinality)
        totalTime = (
            model_conversion_time_per_record * inputEstimates["cardinality"]
            + inputEstimates["totalTime"]
        )
        totalUSD = (
            model_conversion_usd_per_record * inputEstimates["cardinality"]
...
class ParallelFilterCandidateOp(PhysicalOp):
    ...
    def copy(self):
        return ParallelFilterCandidateOp(
            self.outputSchema,
            self.source,
            self.filter,
            self.model,
