...
from palimpzest.optimizer.primitives import (
    Expression,
    Group,
    LogicalExpression,
    PhysicalExpression,
...
class Rule:
    ...
    @staticmethod
    def substitute(
        logical_expression: LogicalExpression, **kwargs
    ) -> Set[Expression]:
        raise NotImplementedError(
            "Calling this method from an abstract base class."
        )
    ...
    def substitute(
        ...
        **kwargs,
        ...
        for input_group_id in logical_expression.input_group_ids:
            ...
            if any(
                [
                    field not in input_group.fields
                    for field in filter_operator.depends_on
                ]
            ):
                continue
            ...
            for expr in logical_exprs:
                ...
                if not (
                    isinstance(expr.operator, ConvertScan)
                    or isinstance(expr.operator, FilteredScan)
                ):
                    continue
                ...
                if any(
                    [
                        field in expr.generated_fields
                        for field in filter_operator.depends_on
                    ]
                ):
                    continue
                ...
                new_generated_fields = (
                    logical_expression.generated_fields.copy()
                )
                ...
                if new_filter_expr.get_expr_id() in expressions:
                    group_id = expressions[
                        new_filter_expr.get_expr_id()
                    ].group_id
                ...
                new_expr = LogicalExpression(
                    ...
                    + [
                        g_id
                        for g_id in logical_expression.input_group_ids
                        if g_id != input_group_id
                    ],
...
class CodeSynthesisConvertRule(ImplementationRule):
    ...
    def substitute(
        ...
        op = cls.physical_convert_class(
            ...
            conventional_fallback_model=physical_op_params[
                "conventional_fallback_model"
            ],
            prompt_strategy=PromptStrategy.DSPY_COT_QA,
            **op_kwargs,
        )
...
class NonLLMFilterRule(ImplementationRule):
    ...
    @staticmethod
    def substitute(
        logical_expression: LogicalExpression, **physical_op_params
    ) -> Set[PhysicalExpression]:
        logical_op = logical_expression.operator
        op_kwargs = logical_op.get_op_params()
        op_kwargs.update(
...
class AggregateRule(ImplementationRule):
    ...
    def substitute(
        ...
        if logical_op.aggFunc == AggFunc.COUNT:
            ...
            raise Exception(
                f"Cannot support aggregate function: {logical_op.aggFunc}"
            )
...
class BasicSubstitutionRule(ImplementationRule):
    ...
    def substitute(
        ...
        physical_op_class = cls.LOGICAL_OP_CLASS_TO_PHYSICAL_OP_CLASS_MAP[
            logical_op.__class__
        ]
