...
class SemanticAggregate(AggregateOp):
    ...
    def __init__(self, agg_str: str, model: Model, prompt_strategy: PromptStrategy = PromptStrategy.COT_AGG, reasoning_effort: str | None = None, *args, **kwargs):
        # call parent constructor
        super().__init__(*args, **kwargs)
    ...
    def is_image_aggregation(self) -> bool:
    ...
    def __call__(self, candidates: list[DataRecord]) -> DataRecordSet:
        ...
        dr = DataRecord.from_agg_parents(schema=self.output_schema, parent_records=candidates)
        ...
        record_op_stats = RecordOpStats(
            ...
            image_operation=self.is_image_aggregation(),
            op_details={k: str(v) for k, v in self.get_id_params().items()},
        )
