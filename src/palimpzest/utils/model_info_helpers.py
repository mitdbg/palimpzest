...
def get_api_key_env_var(model_name: str) -> str | None:
    # 1. Derive provider from the full model name
    model_provider = get_model_provider(model_name)
    
    # 2. Special handling for Google/Vertex
    if model_provider in ["gemini", "vertex_ai"]:
        ...
        return "GOOGLE_API_KEY" # Default fallback

    # 3. Special handling for vLLM (often uses OpenAI protocol)
    if model_provider == "hosted_vllm":
        if os.getenv("VLLM_API_KEY"):
            return "empty-api-key"

    # 4. Standard 1-to-1 mapping
    provider_to_env_var = {
