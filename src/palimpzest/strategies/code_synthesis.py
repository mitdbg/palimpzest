...
from palimpzest.utils import API, getChampionModel
...
class LLMConvertCodeSynthesis(convert.LLMConvert):
    ...
    def __init__(self, 
        ...
        # NOTE: for now, I would not treat the following as hyperparameters of the strategy,
        #       but rather strategy-oriented variables (e.g. we the strategy designer chooses
        #       to the champion model because they want the best possible exemplars)
        self.prompt_strategy = PromptStrategy.DSPY_COT_QA

        # we use the best available model for generating exemplars
        champion_model = getChampionModel()
        self.exemplar_generation_model = champion_model

        # Deus ex machina: we use GPT-3.5 -> Mixtral -> Gemini for the fallback model
        self.conventional_fallback_model = self._get_fallback_model()

        # initialize optimization-specific parameters
        self.field_to_code_ensemble = None
        self.exemplars = None
        self.code_synthesized = False
        self.champion_llm = generators.CustomGenerator(model_name=champion_model.value)
