...
class PatchGeneration(dspy.Signature):
    ...
    Guidelines: 
    1. Only return the diff string without any extra text or explanation. 
    2. Make sure the patch has indentation that matches the codebase, no extra syntax, and can be immedietly applied to the git apply command.
    3. The patch should only modify a single file
    """

    instance_id: str = dspy.InputField(desc="An execution identifier used as an argument for tools")
    bug_report: str = dspy.InputField(desc="A detailed json formatted report of the potential root cause of the code issue and how it might be fixed, refering to function, class, and file names. Includes two fields: 'report' and 'files'. The files field contains the main files relevant to the issue.")
    problem_statement: str = dspy.InputField(desc="A description of the bug")
...
class CodeEditorAgentOp(BaseAgentOp):
    ...
    def run_agent(self, candidate: DataRecord) -> dict: 
        ...
        if BaseAgentOp.LOGGING_ENABLED:
            ...
            LOGGER.info(f'Code Editor Trajectory {patch["instance_id"]} (Max Iters: {self.max_iters}, Model: {self.model}): {pretty_trajectory}')
    ...
    def clean_patch(self, patch: str) -> str:
        ...
        # Ensure that it ends with a new line because usually LLM removes it
        patch += '\n'
        
        return patch


    def create_patch(patch_data: dict, instance_id: str) -> str: 
        """
        Generates and tests a succinct GitHub patch string from a dictionary representing diff data,
        returning any errors with the patch. May also minimize the patch size by removing unnecessary context lines.
        Note: Do not foret to include the instance_id as an argument. 
        ...
        - The content should contain indentation that matches the codebase or else the patch will fail. 
        ...
        - Ensure code is of valid syntax
        - Ensure the paths provided exist in the codebase 
        - IMPORTANT: Limit the number of files to 1
        ...
        utils.verify_and_fix_patch_data(patch_data, BaseAgentOp.instance_params, instance_id)
        patch_data = utils.minimize_patch_data(patch_data)
        
        old_path = patch_data["old_path"]
        new_path = patch_data["new_path"]
        ...
        # Attempts to apply the patch to the codebase
        try: 
            ...
            best_match, file_content = utils.fetch_github_code(file_path, owner, repo, base_commit)
            if best_match != file_path:
                return f"File {file_path} not found in the repo. Did you mean {best_match}?"
            
            # Test Patch
            test_result  = CodeEditorAgentOp.test_patch(patch, file_path, file_content)
            if test_result["returncode"] != 0:
                if test_result['stderr']: 
                    return f"Patch Failed to be Applied ({test_result['stderr']}): \n {patch}"
                else: 
                    return f"Patch Failed to be Applied ({test_result['stdout']}): \n {patch}"
            else:
                return patch
        except Exception as e: 
            print(f'Patch testing failed for {instance_id}: {e}')
            return patch
    
    def test_patch(patch_str, target_file_rel_path, file_content, timeout_seconds=20):
        with tempfile.TemporaryDirectory() as temp_dir:
            # Write the target file
            full_file_path = os.path.join(temp_dir, target_file_rel_path)
            utils.write_target_file(full_file_path, file_content)
            ...
            try:
                # Apply the patch using subprocess with a timeout
                result = subprocess.run(
                    ["patch", "--fuzz=5", "-p1", "-l", "-i", patch_file_path],
                    cwd=temp_dir,
                ...
                print(f'Patch testing result: {result}')
                ...
                print(f'Patch application timed out after {timeout_seconds} seconds.')
                return {
                    "returncode": -1,
                    "stdout": "",
