...
class Variable(pz.Schema):
    ...
    name = pz.Field(
        desc="The label used for a the scientific variable, like a, b, 𝜆 or 𝜖, NOT None",
        required=True,
    )
    description = pz.Field(
        desc="A description of the variable, optional, set 'null' if not found",
        required=False,
    )
    value = pz.Field(
        desc="The value of the variable, optional, set 'null' if not found",
        required=False,
    )
...
if __name__ == "__main__":
    ...
    if run_pz:
        ...
        output = excerpts.convert(
            Variable,
            desc="A variable used or introduced in the paper snippet",
