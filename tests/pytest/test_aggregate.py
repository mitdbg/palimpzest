...
def create_input_record(input_schema: type[BaseModel], idx: int) -> DataRecord:
    ...
    if all(field in input_schema.model_fields for field in TextInputSchema.model_fields):
        dr = DataRecord(input_schema, source_indices=[idx])
        dr.text = f"This record contains the age of an elephant named {idx_to_elephant_name[idx]}."
        dr.age = idx + 1
    if all(field in input_schema.model_fields for field in ImageInputSchema.model_fields):
        dr = DataRecord(input_schema, source_indices=[idx])
        dr.image_file = "tests/pytest/data/elephant.png"
        dr.height = idx_to_elephant_height[idx]
    if all(field in input_schema.model_fields for field in AudioInputSchema.model_fields):
        dr = DataRecord(input_schema, source_indices=[idx])
        dr.audio_file = "tests/pytest/data/elephant.wav"
        dr.year = idx_to_elephant_year[idx]
...
def test_aggregate(mocker, input_schema, physical_op_class):
    ...
    elif all(field in input_schema.model_fields for field in AudioInputSchema.model_fields):
