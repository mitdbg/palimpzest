...
@pytest.fixture
def simple_plan_expected_operator_estimates(
    simple_plan_scan_data, simple_plan_convert_data, simple_plan_filter_data
):
    ...
    expected_operator_estimates[scan_op_id]["time_per_record"] = sum(
        scan_time_per_records
    ) / len(scan_time_per_records)
    ...
    convert_total_output_tokens = simple_plan_convert_data[
        "total_output_tokens"
    ]
    for model_name in all_model_names:
        ...
        expected_operator_estimates[convert_op_id][model_name][
            "time_per_record"
        ] = (
            sum(convert_time_per_records[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[convert_op_id][model_name][
            "cost_per_record"
        ] = (
            sum(convert_cost_per_records[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[convert_op_id][model_name][
            "total_input_tokens"
        ] = (
            sum(convert_total_input_tokens[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[convert_op_id][model_name][
            "total_output_tokens"
        ] = (
            sum(convert_total_output_tokens[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[convert_op_id][model_name][
            "selectivity"
        ] = 1.0
        expected_operator_estimates[convert_op_id][model_name]["quality"] = (
            expected_quality
        )
    ...
    for model_name in all_model_names:
        ...
        num_samples = model_end_idx - model_start_idx
        expected_operator_estimates[filter_op_id][model_name][
            "time_per_record"
        ] = (
            sum(filter_time_per_records[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[filter_op_id][model_name][
            "cost_per_record"
        ] = (
            sum(filter_cost_per_records[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[filter_op_id][model_name][
            "total_input_tokens"
        ] = (
            sum(filter_total_input_tokens[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[filter_op_id][model_name][
            "total_output_tokens"
        ] = (
            sum(filter_total_output_tokens[model_start_idx:model_end_idx])
            / num_samples
        )
        expected_operator_estimates[filter_op_id][model_name]["selectivity"] = (
            expected_selectivity
        )
        expected_operator_estimates[filter_op_id][model_name]["quality"] = (
            expected_quality
        )
...
@pytest.fixture
def simple_plan_expected_results_factory(
    ...
    def expected_results_generator(convert_model, filter_model):
        ...
        def expected_results_fn(physical_op, input_cardinality):
            ...
            if isinstance(physical_op, MarshalAndScanDataOp):
                scan_time_per_record = simple_plan_expected_operator_estimates[
                    "scan123"
                ]["time_per_record"]
            ...
            # compute expected cost, time, and quality for convert operation
                ...
                convert_time_per_record = (
                    simple_plan_expected_operator_estimates["convert123"][
                        convert_model
                    ]["time_per_record"]
                )
                expected_op_time = convert_time_per_record * input_cardinality

                convert_cost_per_record = (
                    simple_plan_expected_operator_estimates["convert123"][
                        convert_model
                    ]["cost_per_record"]
                )
                ...
                expected_op_quality = simple_plan_expected_operator_estimates[
                    "convert123"
                ][convert_model]["quality"]
                convert_selectivity = simple_plan_expected_operator_estimates[
                    "convert123"
                ][convert_model]["selectivity"]
            ...
            # compute expected cost, time, and quality for filter operation
                ...
                filter_time_per_record = (
                    simple_plan_expected_operator_estimates["filter123"][
                        filter_model
                    ]["time_per_record"]
                )
                expected_op_time = filter_time_per_record * input_cardinality

                filter_cost_per_record = (
                    simple_plan_expected_operator_estimates["filter123"][
                        filter_model
                    ]["cost_per_record"]
                )
                ...
                expected_op_quality = simple_plan_expected_operator_estimates[
                    "filter123"
                ][filter_model]["quality"]
                filter_selectivity = simple_plan_expected_operator_estimates[
                    "filter123"
                ][filter_model]["selectivity"]
