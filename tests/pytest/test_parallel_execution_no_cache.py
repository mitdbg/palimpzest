...
class TestSingleThreadExecutionNoCache:
    ...
    def test_execute_plan_with_image_convert(self, execution_engine, real_estate_listing_datasource, real_estate_listing_files_schema, image_real_estate_listing_schema):
        ...
        listing_to_modern_sunlight = {
            "listing1": (True, True),
            "listing2": (False, True),
            "listing3": (False, False),
        }
        dr = output_records[0]
        assert dr.listing in expected_listings
        modern, sunlight = listing_to_modern_sunlight[dr.listing]
        assert getattr(dr, 'is_modern_and_attractive', None) == modern
        assert getattr(dr, 'has_natural_sunlight', None) == sunlight
